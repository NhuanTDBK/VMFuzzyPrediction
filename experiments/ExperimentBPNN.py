...
def experiment(sliding_number=3, hidden_node=15):
    ...
    estimator = NeuralFlowRegressor(learning_rate=0.01, hidden_nodes=[hidden_node], steps=5000, optimize='Adam')
    ...
    np.savez('model_saved/BPNN_%s_%s' % (sliding_number, score_rmse), y_pred=y_pred, y_true=y_actual_test)
