...
neuralNet = NeuralFlowRegressor(learning_rate=1E-04,steps=5000)
# estimator = GAEstimator(cross_rate=0.7, mutation_rate=0.01)
estimator = ACOEstimator(Q=0.7,epsilon=0.1,number_of_solutions=100)
...
score_nn = mean_squared_error(y_pred=y_pred,y_true=y_test[n_sliding_window-2:])
print score_nn
np.savez('../model_saved/GABPNN_%s_sliding_%s'%(score_nn,n_sliding_window),y_test=y_test, y_pred=y_pred)
plot_figure(y_pred=y_pred,y_true=y_test[1:],title="BPNN for RAM metric Sliding window  = %s, rmse = %s, hidden nodes = %s "%(n_sliding_window,score_nn,n_hidden))
# score_list = {}
