...
scaler = MinMaxScaler()
print 'FACO Model'
dat_usage = pd.read_csv('sample_610_10min.csv')['cpu_rate'][:2400].round(4)
...
neuralNet = NeuralFlowRegressor(learning_rate=1E-05,verbose=1,steps=1000,activation='sigmoid')
...
np.savez('FACO_%s_sliding_%s'%(score_fuzz,sliding_number),y_test=y_test[:190], y_pred=ft[:190])
# <codecell>
