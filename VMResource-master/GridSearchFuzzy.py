...
def gridSearchSliding(sliding_number):
    ...
    for loop in np.arange(11):
        ...
        neuralNet = NeuralFlowRegressor(learning_rate=1E-04,verbose=1,steps=1000,activation='sigmoid')
    ...
    np.savez('FACO_%s_sliding_%s'%(score_fuzz,sliding_number),y_pred=ft[:190],y_test=y_test[:190])
